...
class Controller
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        Config::set('shop_info',tpCache('shop_info'));
...
//        }
        ...
        if ($this->beforeActionList) {
			...
			$this->beforeActionMethod($method, $options);
		}
    }
     ...
     * @return void
     */
    protected function beforeActionMethod($method, $options = [])
    {
        if (isset($options['only']) &&  $options['only'] == 'except') {
            if (is_string($options['only'])) {
                $options['only'] = explode(',', $options['only']);
            }
        ...
        } elseif (isset($options['except'])  &&  $options['only'] == 'protected') {
            if (is_string($options['except'])) {
                $options['except'] = explode(',', $options['except']);
            }
