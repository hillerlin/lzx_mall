<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:45:"./template/pc/rainbow/order\order_detail.html";i:1568702283;s:59:"D:\TPshop_tp5_20210401\template\pc\rainbow\user\header.html";i:1568702284;s:61:"D:\TPshop_tp5_20210401\template\pc\rainbow\public\footer.html";i:1568702283;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<div class="home-index-middle">
    <div class="w1224">
                ...
                <div class="wacheng fl">
                    ...
                    <?php if($order['prom_type'] == 4): ?>
                        ...
                        <?php if($order['pre_sell']['deposit_price'] > 0): ?>
                            <p class="ddn1"><span>尾款支付开始时间：</span>
                                <span><?php echo date('Y-m-d H:i:s',$order['pre_sell']['pay_start_time']); ?></span>
                            </p>
                            <p class="ddn1"><span>尾款支付截止时间：</span>
                                <span><?php echo date('Y-m-d H:i:s',$order['pre_sell']['pay_end_time']); ?></span>
                            </p>
                        ...
                        <?php endif; if($order['pre_sell']['is_finished'] == 2): if(time() > $order['pre_sell']['pay_end_time'] AND $order['pay_status'] != 1): ?>
                                ...
                                <h3 style="font: 700 24px/34px 'Microsoft YaHei';color: #e4393c; padding-top:20px;">订单关闭</h3>
                            <?php endif; endif; endif; if($order['pay_btn'] == 1): ?>
                        <h3 style="font: 700 24px/34px 'Microsoft YaHei';color: #e4393c; padding-top:20px;">等待付款</h3>
                        <a class="ddn3" style="margin-top:20px;" href="<?php echo U('Home/Cart/cart4',array('order_id'=>$order[order_id])); ?>">立即付款</a>
                    <?php else: if(!(empty($childOrder) || (($childOrder instanceof \think\Collection || $childOrder instanceof \think\Paginator ) && $childOrder->isEmpty()))): ?>
							...
							<?php if(is_array($childOrder) || $childOrder instanceof \think\Collection || $childOrder instanceof \think\Paginator): if( count($childOrder)==0 ) : echo "" ;else: foreach($childOrder as $key=>$vo): ?>
								<p class="ddn1"><a href="<?php echo U('Order/order_detail',['id' => $vo['order_id']]); ?>"><?php echo $vo['order_sn']; ?></a></p>
                    ...
                    <?php endif; if($order['cancel_btn'] == 1): if($order[pay_status] == 0): ?>
                        ...
                        <?php else: ?>
                            <a class="consoorder ddn3" href="javascript:void(0);" data-url="<?php echo U('Home/Order/refund_order',array('order_id'=>$order[order_id])); ?>" onClick="refund_order(this)" >取消订单</a>
                        <?php endif; endif; if($order['pay_tail_btn'] == 1): ?>
                        <a class="ddn3" style="margin-top:20px;" href="<?php echo U('/Home/Cart/cart4',array('order_id'=>$order[order_id])); ?>">支付尾款</a>
                    <?php endif; ?>

                   <!-- <p class="ddn4"><a href="<?php echo U('order_detail',array('id'=>$order[order_id],'act'=>'print')); ?>" target="_blank"><i class="y-comp6"></i>打印订单</a></p>-->
                </div>
                                ...
                                <div class="aloinfe fime1">
                                    ...
                                    <div class="ddfon">
                                        ...
                                        <?php if($order[pay_time] > 0): ?>
                                            <p><?php echo date('Y-m-d H:i:s',$order['pay_time']); ?></p>
                                        <?php endif; if($order['prom_type'] == 4 AND $order['pre_sell']['deposit_price'] > 0): ?>
                                            尾款支付<?php echo date('Y-m-d H:i:s',$order['pre_sell']['pay_start_time']); ?>至<?php echo date('Y-m-d H:i:s',$order['pre_sell']['pay_end_time']); endif; ?>
                                    </div>
                                </div>
                            </li>
                                ...
                                <div class="aloinfe fime2">
                                    ...
                                    <div class="ddfon">
                                        ...
                                        <?php if($order[shipping_time] > 0): ?>
                                            <p><?php echo date('Y-m-d',$order['shipping_time']); ?></p>
                                            <p><?php echo date('H:i:s',$order['shipping_time']); ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </li>
                                ...
                                <div class="aloinfe fime4">
                                    ...
                                    <div class="ddfon">
                                        ...
                                        <?php if($order[confirm_time] > 0): ?>
                                            <p><?php echo date('Y-m-d H:i:s',$order['confirm_time']); ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </li>
                        ...
                        <div class="wacheng fl">
                            <div class="shioeboixe">
                                <div class="sohstyle p">
                                    <div class="odjpyes">
                                        <img src="/template/pc/rainbow/static/images/kuaidi.jpg"/>
                                    </div>
                                    <div class="osnhptek">
                ...
                <script>
                    $(function() {
                        $.ajax({
                            type : "GET",
                            dataType: "json",
                            url:"<?php echo U('home/api/queryExpress'); ?>?shipping_code=<?php echo $v['shipping_code']; ?>&invoice_no=<?php echo $v['invoice_no']; ?>",//+tab,
            ...
            <div class="rshrinfmas">
                ...
                <div class="spff spff-two">
                    ...
                    <div class="psbaowq">
                        ...
                        <p><span class="fircl">付款时间：</span><span class="lascl"><?php if($order[pay_status] > 0): ?><?php echo date('Y-m-d H:i:s',$order['pay_time']); else: ?>未支付<?php endif; ?></span></p>
                        <?php if($order['prom_type'] != 4): ?>
                        ...
                        <?php if($order['prom_type'] == 4 AND $order['paid_money'] > 0): if($order['pay_status'] == 1): ?>
                            ...
                            <tr>
                                <p><span class="fircl">尾款支付时间：</span><span class="lascl"><?php echo date('Y-m-d H:i:s',$order['pre_sell']['pay_start_time']); ?>至<?php echo date('Y-m-d H:i:s',$order['pre_sell']['pay_end_time']); ?></span></p>
                            </tr>
                        <?php endif; ?>
                        <!-- 预售商品 end -->
                    </div>
                ...
                <div class="spff mar0 spff-two">
                    ...
                    <div class="psbaowq">
                        ...
                        <p><span class="fircl">发票抬头：</span><span class="lascl"><?php if(($order['invoice_desc'] != '不开发票')): ?>
                            <?php echo !empty($order['invoice_title'])?$order['invoice_title']:''; endif; ?></span></p>
        ...
        <div class="beenovercom">
            <div class="shoptist">
                <span><?php echo $tpshop_config['shop_info_store_name']; ?><a href="tencent://message/?uin=<?php echo $tpshop_config['shop_info_qq']; ?>&Site=<?php echo \think\Config::get('shop_info.copyright'); ?>商城&Menu=yes" target="_blank"><i class="y-comp9"></i></a></span>
            </div>
                            ...
                            <td class="sx6">
                                <div class="twrbac">
                                    <?php if(($order['comment_btn'] == 1) and ($goods['is_comment'] == 0) and ($goods['is_send'] < 3)): ?>
                                        <a href="<?php echo U('Home/Order/comment_list',array('order_id'=>$order['order_id'],'rec_id'=>$goods['rec_id'])); ?>">评价</a>
        ...
        <div class="numzjsehe">
            ...
            <p><span class="sp_tutt">价格调整：</span><span class="smprice"><em>￥</em>
                <?php if($order['discount'] > 0): ?>
                    ...
                    <?php echo abs($order['discount']); endif; ?>
                </span></p>
