<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:43:"./template/pc/rainbow/order\order_list.html";i:1616701750;s:52:"H:\www\lzx_mall\template\pc\rainbow\user\header.html";i:1616701752;s:50:"H:\www\lzx_mall\template\pc\rainbow\user\menu.html";i:1616701752;s:54:"H:\www\lzx_mall\template\pc\rainbow\public\footer.html";i:1616701750;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
                    ...
                    <?php if(empty($order_list) || (($order_list instanceof \think\Collection || $order_list instanceof \think\Paginator ) && $order_list->isEmpty())): ?>
                        ...
                        <?php else: if(is_array($order_list) || $order_list instanceof \think\Collection || $order_list instanceof \think\Paginator): $i = 0; $__LIST__ = $order_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?>
                            <table width="100%" border="" cellspacing="" cellpadding="">
                                        ...
                                        <div class="fl_ttmm">
                                            ...
                                            <div class="paysoon">
                                                ...
                                                <?php endif; if($order['receive_btn'] == 1 && $order['pay_status'] == 1): ?>
                                                    <a class="ps_lj" href="javascript:;" onclick="order_confirm(<?php echo $order['order_id']; ?>);">确认收货</a><?php endif; if($orders['cancel_btn'] == 1): if($order[pay_status] == 0): ?>
                                                    <a class="consoorder" href="javascript:void(0);" onClick="cancel_order(<?php echo $order['order_id']; ?>)" >取消订单</a>
                                                <?php else: ?>
                                                	<a class="consoorder" href="javascript:void(0);" data-url="<?php echo U('Home/Order/refund_order',['order_id'=>$order['order_id']]); ?>" onClick="refund_order(this)" >取消订单</a>
                                                	<?php endif; else: ?>
                                                    <!--<div class="dele" onclick="verConfirm('确认删除该订单?','<?php echo U('Home/Order/del_order',array('order_id'=>$order['order_id'], 'type'=>\think\Request::instance()->param('type'))); ?>')"></div>-->
                                                <?php endif; ?>
                                                <!--<div class="dele"></div>-->
                                            </div>
                                        </div>
                                ...
                                <?php if(is_array($order['order_goods']) || $order['order_goods'] instanceof \think\Collection || $order['order_goods'] instanceof \think\Paginator): $k = 0; $__LIST__ = $order['order_goods'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($k % 2 );++$k;if($k == 1): ?>
                                        <tr class="conten_or">
                                            <td class="sx1">
                                            ...
                                            <td class="sx5" rowspan="<?php echo count($order['order_goods']); ?>">
                                                <div class="detail_or">
                                                    <p class="d_yzo">
                                                        <?php echo $order['order_status_detail']; ?>
                                                    </p>
                                                    <p>
                                                        <?php if($order[prom_type] == 5): ?>
                                                            <a href="<?php echo U('Order/virtual_order',array('order_id'=>$order['order_id'])); ?>">查看详情</a>
                                                        <?php else: ?>
                                                            <a href="<?php echo U('Order/order_detail',array('id'=>$order['order_id'])); ?>">查看详情</a>
                                                        <?php endif; ?>
                                                    </p>
                                                </div>
                                            </td>
                                            <td class="sx6" rowspan="<?php echo count($order['order_goods']); ?>">
                                                <div class="rbac">
                                                    <p class=""><a class="buy_order" data-order-id="<?php echo $order['order_id']; ?>" href="javascript:;">再次购买</a></p>
                                                    <?php if(($order['comment_btn'] == 1) and ($goods[is_comment] == 0)): ?>
                                                        <p class="inspect">
                                    ...
                                    <?php else: ?>
                                        <tr class="conten_or">
                                            ...
                                            <td class="sx2"><span>￥</span><span><?php echo $goods['prom_type']==7?$goods['member_goods_price']:$goods['final_price']; ?></span></td>
                                            <td class="sx3">
                                                <span>x<?php echo $goods['goods_num']; ?></span>
                                                <?php if(!(empty($goods['return_goods']) || (($goods['return_goods'] instanceof \think\Collection || $goods['return_goods'] instanceof \think\Paginator ) && $goods['return_goods']->isEmpty()))): ?>
                                                    <a class="applyafts">已申请售后</a>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.buy_order', function() {
        ...
        $.ajax({
            type : "post",
            url : "<?php echo U('Home/Order/oneMore'); ?>",
            data:{order_id:order_id},
            dataType : 'json',
            success : function(data){
                var err_html = '';
