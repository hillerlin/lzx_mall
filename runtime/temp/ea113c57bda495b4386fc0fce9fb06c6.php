    ...
    <div class="w1224">
        <div class="search-path fl">
            <?php if(is_array($goods['goods_category']['parent_list']) || $goods['goods_category']['parent_list'] instanceof \think\Collection || $goods['goods_category']['parent_list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $goods['goods_category']['parent_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category_parent): $mod = ($i % 2 );++$i;?>
                <a href="<?php echo U('/Home/Goods/goodsList',array('id'=>$category_parent['id'])); ?>"><?php echo $category_parent['name']; ?></a>
                <i class="litt-xyb"></i>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            <a href="<?php echo U('/Home/Goods/goodsList',array('id'=>$goods['goods_category']['id'])); ?>"><?php echo $goods['goods_category']['name']; ?></a>
            <i class="litt-xyb"></i>
            <div class="havedox">
        ...
        <?php if((!empty($tpshop_config['basic_im_choose'])) && ($tpshop_config['basic_im_choose'] == 1)): ?>
            ...
            <div class="online-service fr p">
                <a href="javascript:;" class="z-onlines z-online-service fr"  user_id="<?php echo \think\Session::get('user.user_id'); ?>" uname="<?php echo \think\Session::get('user.nickname'); ?>"
                   avatar="<?php echo \think\Session::get('user.head_pic'); ?>" sign="" goods_id="<?php echo $goods['goods_id']; ?>" web_id="<?php echo SITE_URL; ?>" im_href="<?php echo $tpshop_config['basic_im_website']; ?>" id="workerman-kefu" onclick="jump()" ws_socket="<?php echo $tpshop_config['basic_ws_socket']; ?>">
                ...
                <div class="shop-price-cou p">
                                ...
                                <span class="mobile-buy-cheap">
                                    ...
                                    <i>
                                        ...
                                        <img class="big-qrcode-h"
                                             img-url="/index.php?m=Home&c=Index&a=qr_code&data=<?php echo U('Mobile/Goods/goodsInfo',['id'=>$goods['goods_id']],true,true); ?>&head_pic=<?php echo $head_pic; ?>&back_img=<?php echo $back_img; ?>"
                                             alt=""/>
                                    </i>
                                </span>
                            </li>
                    ...
                    <div class="shop-cou-ri">
                        ...
                        <div class="allcomm"><p>累计销量</p>
                            <p class="f_blue"><?php echo $goods['sales_sum']+$goods['virtual_sales_sum']; ?></p></div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function addCombinationShop(e,t) {
        ...
        if(n==0){
            ...
            $.ajax({
                type: "Post",
                url: "<?php echo U('Home/Cart/addCombination'); ?>",
                ...
                <div class="tjhot-shoplist type-bot">
                    <?php
                                ...
                                {                            
                                    ...
                                    S("sql_".$md5_key,$sql_result_vo,31104000);
                                }    
                              foreach($sql_result_vo as $k=>$vo): ?>
                        <div class="alone-shop">
                            <a href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$vo[goods_id])); ?>"><img
                                    src="<?php echo goods_thum_images($vo['goods_id'],206,206); ?>" style="display: inline;"></a>
                            <p class="line-two-hidd"><a href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$vo[goods_id])); ?>"><?php echo getSubstr($vo['goods_name'],0,30); ?></a>
                            </p>
...
<div class="soubao-sidebar">
    ...
    <div class="sidertabs tab-lis-2">
        <div class="icon-tabe-chan advice">
            <a title="点击这里给我发消息" href="tencent://message/?uin=<?php echo $tpshop_config['shop_info_qq2']; ?>&amp;Site=<?php echo \think\Config::get('shop_info.copyright'); ?>商城&amp;Menu=yes" target="_blank">
                <i class="share-side share-side1"></i>
                <!--<i class="share-side tab-icon-tip"></i>-->
...
<div style="display: none" id="look_see">
    <?php
                                ...
                                $result_name = $sql_result_look = S("sql_".$md5_key);
                                if(empty($sql_result_look))
                                {                            
                                    $result_name = $sql_result_look = \think\Db::query("select goods_id,goods_name,shop_price from `__PREFIX__goods` where goods_id != $goods[goods_id] AND cat_id = $goods[cat_id] AND is_on_sale = 1 limit 12"); 
                                    S("sql_".$md5_key,$sql_result_look,31104000);
                                }    
                              foreach($sql_result_look as $k=>$look): ?>
        <div class="tjhot-shoplist type-bot">
            <div class="alone-shop">
                <a href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$look[goods_id])); ?>">
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $(document).on('click', '#buy_now', function () {
            ...
            if (getCookie('user_id') == '') {
                pop_login("<?php echo \think\Config::get('shop_info.copyright'); ?>");
                return;
            }
            var is_virtual = $("input[name='is_virtual']").val();//是否是虚拟商品
            ...
            if (is_virtual == 1) {
                ...
                if ((goods_num <= store_count && goods_num <= virtual_limit) || (goods_num < store_count && virtual_limit == 0)) {
                    ...
                    // form.attr('action', "<?php echo U('Home/Virtual/buy_virtual'); ?>").submit();//之前的跳转虚拟订单跳转页
                } else {
    ...
    function altergoodsnum(n) {
        ...
        if (price_ladder != null && price_ladder != '') {
            $.ajax({
                type: "POST",
                url: "<?php echo U('Home/Goods/priceladder'); ?>",
                dataType: 'json',
