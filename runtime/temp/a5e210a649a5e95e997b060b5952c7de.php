<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:38:"./template/pc/rainbow/index\index.html";i:1568702283;s:65:"D:\wamp\www\tpshop_cyb\www\template\pc\rainbow\public\header.html";i:1568702283;s:65:"D:\wamp\www\tpshop_cyb\www\template\pc\rainbow\public\footer.html";i:1568702283;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<div id="myCarousel" class="carousel clearfix">
	<ul class="carousel-inner">
        <?php $pid =2;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time <= 1569546000 and end_time >= 1569546000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("5")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v):       
    ...
    ?>
		<li class="item" style="background:<?php echo $v['bgcolor']; ?>;">
			<a class="item-pic" href="<?php echo $v['ad_link']; ?><?php echo !empty($edit_ad)?'&suggestion=1920*480' : ''; ?>" <?php if($v['target'] == 1): ?>target="_blank"<?php endif; ?>>
			<img class="w-100" src="<?php echo $v[ad_code]; ?>" title="<?php echo $v[title]; ?>" alt="<?php echo $v[title]; ?>">
		</a>
		</li>
		<?php endforeach; ?>
	</ul>
...
<!-- 	<div class="banner-right-box">
	<?php $pid =52;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time <= 1569546000 and end_time >= 1569546000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("2")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$vb):       
    ...
    ?>
		<a class="banner-right-item t-all" href="<?php echo $vb[ad_link]; ?>&suggestion=200*210"><img src="<?php echo $vb[ad_code]; ?>" alt="<?php echo $vb[title]; ?>" /></a>
	<?php endforeach; ?>
	</div> -->
</div>

<div class="adv3 w1224 clearfix">
	<?php $pid =50;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time <= 1569546000 and end_time >= 1569546000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("3")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$vr):       
    ...
    ?>
	<a class="recommend-brand t-all" href="<?php echo $vr[ad_link]; ?><?php echo !empty($edit_ad)?'&suggestion=350*180' : ''; ?>">
...
<div class="rmtj">
	...
	<div class="rmtj-cont">
		<div class="rmtj-list-main slider1">
			<?php if(is_array($cateList) || $cateList instanceof \think\Collection || $cateList instanceof \think\Paginator): if( count($cateList)==0 ) : echo "" ;else: foreach($cateList as $k=>$v): if(is_array($v['recommend_goods']) || $v['recommend_goods'] instanceof \think\Collection || $v['recommend_goods'] instanceof \think\Paginator): if( count($v['recommend_goods'])==0 ) : echo "" ;else: foreach($v['recommend_goods'] as $gk=>$rg): ?>
			<div class="rmtj-list" onclick="to_url(this)" data-url="<?php echo U('Home/Goods/goodsInfo',array('id'=>$rg[goods_id])); ?>">
				<a href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$rg[goods_id])); ?>">
				<div class="img">
					<img src="<?php echo goods_thum_images($rg[goods_id],200,200); ?>" alt="">
				</div>
				<p class="rmtj-title"><?php echo getSubstr($rg[goods_name],0,35); ?></p>
				<p class="rmtj-price">￥<?php echo $rg[shop_price]; ?></p></a>
			</div>
...
<?php $pid =49;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time <= 1569546000 and end_time >= 1569546000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v5):       
    ...
    ?>
	<a href="<?php echo $v5[ad_link]; ?><?php echo !empty($edit_ad)?'&suggestion=1200*120' : ''; ?>" class="adver_line" ad_id="<?php echo $v5['ad_id']; ?>">
		<img src="<?php echo $v5[ad_code]; ?>" alt="<?php echo $v5[title]; ?>"/>
	</a>
...
<div class="floor floor<?php echo $k+1; ?> w1224">
	...
	<div class="floor-main">
		<div class="floor-brand">
			<?php $pid =12+$k;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time <= 1569546000 and end_time >= 1569546000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$vl):       
    ...
    ?>
			<a class="brand-big" href="<?php echo $vl[ad_link]; ?><?php echo !empty($edit_ad)?'&suggestion=240*436' : ''; ?>"  ad_id="<?php echo $vl['ad_id']; ?>"><img class="w-100" src="<?php echo $vl[ad_code]; ?>" alt="<?php echo $vl[title]; ?>" /></a>
			<?php endforeach; $pid =32+$k;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time <= 1569546000 and end_time >= 1569546000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$vs):       
    ...
    ?>
			<a class="brand-samll" href="<?php echo $vs[ad_link]; ?><?php echo !empty($edit_ad)?'&suggestion=240*124' : ''; ?>"  ad_id="<?php echo $vs['ad_id']; ?>"><img class="w-100" src="<?php echo $vs[ad_code]; ?>" alt="<?php echo $vs[title]; ?>" /></a>
			<?php endforeach; ?>
		</div>
		<div class="floor-goods-list">
			...
			<a class="floor-goods-item" href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$g[goods_id])); ?>">
				...
				<div class="googs-title ellipsis-1"><?php echo $g[goods_name]; ?></div>
...
<div class="slidebar-right">
	<?php if((!empty($tpshop_config['basic_im_choose'])) && ($tpshop_config['basic_im_choose'] == 1)): ?>
		<link rel="stylesheet" href="http://<?php echo $tpshop_config['basic_im_website']; ?>/static/test/common/layui/css/layui.css" media="all">
		<!--IM客服-->
		<a class="slidebar-item ico-slidebar1" user_id="<?php echo \think\Session::get('user.user_id'); ?>" uname="<?php echo \think\Session::get('user.nickname'); ?>"
		   avatar="<?php echo \think\Session::get('user.head_pic'); ?>" sign="" goods_id="<?php echo $goods['goods_id']; ?>" web_id="<?php echo \think\Request::instance()->host(); ?>" im_href="<?php echo $tpshop_config['basic_im_website']; ?>" id="workerman-kefu" onclick="jump()">
			<div class="sbar-hover-txt">客服服务</div>
		</a>
		<script type="text/javascript" src="http://<?php echo $tpshop_config['basic_im_website']; ?>/static/test/common/layui/layui.js"></script>
		<script type="text/javascript" src="http://<?php echo $tpshop_config['basic_im_website']; ?>/static/test/common/js/main.js"></script>
		<?php else: ?>
		<!--qq客服-->
	<a class="slidebar-item ico-slidebar1" target="_blank" href="tencent://message/?uin=<?php echo $tpshop_config['shop_info_qq2']; ?>&amp;Site=<?php echo \think\Config::get('shop_info.copyright'); ?>商城&amp;Menu=yes" >
		<div class="sbar-hover-txt">客服服务</div>
	</a><?php endif; ?>
