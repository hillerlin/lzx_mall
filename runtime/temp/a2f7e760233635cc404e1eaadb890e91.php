								...
								<?php else: ?>
									...
									<?php if(is_array($order_list) || $order_list instanceof \think\Collection || $order_list instanceof \think\Paginator): $i = 0; $__LIST__ = $order_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
										...
										<table width="100%" border="" cellspacing="" cellpadding="">
											...
											<?php if(is_array($list['order_goods']) || $list['order_goods'] instanceof \think\Collection || $list['order_goods'] instanceof \think\Paginator): $i = 0; $__LIST__ = $list['order_goods'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?>
												<tr class="conten_or">
													<td class="sx1">
															...
															<p class="d_yzo">
																<?php if($goods[is_comment] == 1): ?>
																	...
																	<?php elseif($goods[is_comment] != 1 and $list['shipping_status'] == 2): if($goods[is_send] == 0): ?>
																		...
																		已发货
																	<?php endif; else: ?>
																	<?php echo $list['OrderStatusDetail']; endif; ?>
                                                            </p>
                                                            <p>
        ...
        function cancel_order(order_id){
            layer.confirm("确定取消订单?",{
                ...
                $.ajax({
                    ...
                    url : '/index.php?m=Home&c=Order&a=cancel_order&id='+order_id,
                    dataType : 'json',
                    success : function(data){
                        if(data.status == 1){
                            location.href = "/index.php?m=Home&c=Order&a=order_detail&id="+order_id;
                        }else{
